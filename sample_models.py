...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for idx in range(recur_layers):
        rnn_layer = GRU(units, activation="relu",
              return_sequences=True, implementation=2, name='rnn%d'%idx)(last_layer)
        # TODO: Add batch normalization 
        bn_rnn_layer = BatchNormalization()(rnn_layer)
        last_layer = bn_rnn_layer
...
def final_model(input_dim, units, recur_layers, output_dim=29):
    ...
    last_layer = input_data
    for idx in range(recur_layers):
        # TODO: Add bidirectional layer 
        bidir_rnn = Bidirectional(GRU(units, activation="relu",
              return_sequences=True, implementation=2, name='rnn%d'%idx))(last_layer)
        bn_rnn_layer = BatchNormalization()(bidir_rnn)
        last_layer = bn_rnn_layer
